...
# Handle injury history
injury_col = "No history of injury/arthritis/RA/AVN=0, sports related=1, fall/fracture=2, Other=3)"
df_metadata[injury_col] = df_metadata[injury_col].astype(str).str.extract(r"^(\d)")
df_metadata = df_metadata.dropna(subset=[injury_col, "Age"])
...
df = df_metadata[[injury_col, "Age", "revision_label"]].dropna()
X = df[[injury_col, "Age"]].astype(float)
y = df["revision_label"].astype(int)
