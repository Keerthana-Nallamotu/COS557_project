...
# Filter metadata to patients who have revision data
df_metadata = df_metadata[df_metadata["ID"].isin(df_revisions["patient_id"])]
df_metadata["revision_label"] = df_metadata["ID"].map(revision_labels)

# Dataframe based on insurance status
insurance_col = "Insurance (Medicare=0, Medicaid=1, PPO/HMO=2)"
df_metadata[insurance_col] = df_metadata[insurance_col].astype(str).str.extract(r"^(\d)")
df_metadata = df_metadata.dropna(subset=[insurance_col])
df_metadata[insurance_col] = df_metadata[insurance_col].astype(int)

# Medicare and Medicaid → 0 (public), PPO/HMO (2) → 1 (private)
df_metadata[insurance_col] = df_metadata[insurance_col].apply(lambda x: 1 if x == 2 else 0)

df = df_metadata[[insurance_col, "revision_label"]].dropna()
X = df[[insurance_col]]
y = df["revision_label"]
